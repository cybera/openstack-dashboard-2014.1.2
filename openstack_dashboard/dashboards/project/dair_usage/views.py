...
class DAIRProjectData(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        query_results = {}
        queries = {
            'project_allocated_instances': "aliasSub(aliasByNode(summarize(keepLastValue(projects.%s.cloud_usage-*.instances,10),'5min','avg'), 2), 'cloud_usage-([^\\n\\r]*)', 'number of instances in \\1')" % project_id,
...
class DAIRInstanceDataCSVSummary(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="Instance_Usage_Summary.csv"'
        w = csv.writer(response)
        ...
        w.writerow([_("timestamp"),_("cpu_time"),_("memory available"),_("memory used"),_("rx_bytes (received)"),_("tx_byes (transmitted)"),_("bytes_used"),_("wr_req (writes)"),_("rd_req (reads)")])
...
class DAIRProjectDataCSVSummary(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="Project_Usage_Summary.csv"'
        w = csv.writer(response)
        ...
        w.writerow([_("timestamp"),_("number of instances in alberta"),_("number of instances in quebec"),_("number of virtual cpus in alberta"),_("number of virtual cpus in quebec"),_("memory usage in alberta"),_("memory usage in quebec"),_("disk usage in alberta"),_("disk usage in quebec")])
